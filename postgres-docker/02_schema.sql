...
CREATE TABLE IF NOT EXISTS system_prompt_templates (
    ...
    article_type VARCHAR(50) CHECK (article_type IN ('general_interest', 'success_case', NULL)),
...
CREATE TABLE IF NOT EXISTS generated_content (
    ...
    model_key_selected VARCHAR(50) NOT NULL,
    actual_llm_model_name_used VARCHAR(100) NOT NULL,
    temperature_used FLOAT NOT NULL,
    max_tokens_article_used INTEGER,
    max_tokens_summary_used INTEGER,
    
    urls_researched JSONB,
    web_research_options_used JSONB,
    pdf_filename_original VARCHAR(255),

    generated_text_main TEXT NOT NULL,
    generated_text_summary TEXT,
    researched_content_summary TEXT,
...
CREATE INDEX IF NOT EXISTS idx_system_prompt_templates_name ON system_prompt_templates(template_name);
CREATE INDEX IF NOT EXISTS idx_system_prompt_templates_module_style ON system_prompt_templates(content_module, article_type, style_key);
...
CREATE INDEX IF NOT EXISTS idx_tags_user_id_tag_name ON tags(user_id, tag_name);
...
-- Para los triggers, no hay un "IF NOT EXISTS" directo en CREATE TRIGGER.
-- Si el script se ejecuta varias veces y el trigger ya existe, dará un error.
-- En un script de inicialización que se espera que corra sobre un esquema "limpio"
-- (o donde las tablas se crean con IF NOT EXISTS), esto suele estar bien.
-- Para una idempotencia total, se necesitaría un bloque DO con una comprobación.
-- Por ahora, lo mantenemos así por simplicidad, asumiendo que el problema principal
-- era la creación de tablas.

DROP TRIGGER IF EXISTS set_timestamp_users ON users; -- Añadido para idempotencia
CREATE TRIGGER set_timestamp_users
BEFORE UPDATE ON users
FOR EACH ROW
EXECUTE FUNCTION trigger_set_timestamp();

DROP TRIGGER IF EXISTS set_timestamp_user_preferences ON user_preferences; -- Añadido
CREATE TRIGGER set_timestamp_user_preferences
BEFORE UPDATE ON user_preferences
FOR EACH ROW
EXECUTE FUNCTION trigger_set_timestamp();

DROP TRIGGER IF EXISTS set_timestamp_system_prompt_templates ON system_prompt_templates; -- Añadido
CREATE TRIGGER set_timestamp_system_prompt_templates
BEFORE UPDATE ON system_prompt_templates
FOR EACH ROW
EXECUTE FUNCTION trigger_set_timestamp();

DROP TRIGGER IF EXISTS set_timestamp_user_custom_prompts ON user_custom_prompts; -- Añadido
CREATE TRIGGER set_timestamp_user_custom_prompts
BEFORE UPDATE ON user_custom_prompts
FOR EACH ROW
EXECUTE FUNCTION trigger_set_timestamp();

DROP TRIGGER IF EXISTS set_timestamp_generated_content ON generated_content; -- Añadido
CREATE TRIGGER set_timestamp_generated_content
BEFORE UPDATE ON generated_content
FOR EACH ROW
EXECUTE FUNCTION trigger_set_timestamp();

-- Comentarios sobre las tablas y columnas
